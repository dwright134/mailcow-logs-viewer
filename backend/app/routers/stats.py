...
async def get_dashboard_stats(db: Session = Depends(get_db)):
    ...
    try:
        ...
        messages_24h = (
            ...
            .filter(MessageCorrelation.first_seen >= day_ago)
            .count()
        )

        messages_7d = (
            db.query(MessageCorrelation)
            .filter(MessageCorrelation.first_seen >= week_ago)
            .count()
        )

        messages_30d = (
            db.query(MessageCorrelation)
            .filter(MessageCorrelation.first_seen >= month_ago)
            .count()
        )
        ...
        blocked_24h = (
            ...
            .filter(
                and_(
                    ...
                    MessageCorrelation.final_status.in_(
                        ["bounced", "rejected", "spam"]
                    ),
                )
            )
            .count()
        ...
        auth_failures_24h = (
            ...
            .filter(and_(NetfilterLog.time >= day_ago, NetfilterLog.rule_id == 3))
            .count()
        )

        auth_failures_7d = (
            db.query(NetfilterLog)
            .filter(and_(NetfilterLog.time >= week_ago, NetfilterLog.rule_id == 3))
            .count()
        )
        ...
        inbound_24h = (
            db.query(RspamdLog)
            .filter(and_(RspamdLog.time >= day_ago, RspamdLog.direction == "inbound"))
            .count()
        )

        outbound_24h = (
            db.query(RspamdLog)
            .filter(and_(RspamdLog.time >= day_ago, RspamdLog.direction == "outbound"))
            .count()
        )

        internal_24h = (
            db.query(RspamdLog)
            .filter(and_(RspamdLog.time >= day_ago, RspamdLog.direction == "internal"))
            .count()
        )

        return {
            "messages": {"24h": messages_24h, "7d": messages_7d, "30d": messages_30d},
            "blocked": {
                "24h": blocked_24h,
                "7d": blocked_7d,
                "percentage_24h": round(
                    (blocked_24h / messages_24h * 100) if messages_24h > 0 else 0, 2
                ),
            },
            "deferred": {"24h": deferred_24h, "7d": deferred_7d},
            "auth_failures": {"24h": auth_failures_24h, "7d": auth_failures_7d},
            "direction": {
...
async def get_timeline_stats(hours: int = 24, db: Session = Depends(get_db)):
    """
    ...
    try:
        ...
        timeline = (
            db.query(
                ...
                func.sum(func.cast(RspamdLog.is_spam, func.Integer)).label(
                    "spam_count"
                ),
            )
            .filter(RspamdLog.time >= cutoff)
            .group_by("hour")
            .order_by("hour")
            .all()
        )
...
async def get_top_spam_triggers(limit: int = 10, db: Session = Depends(get_db)):
    """
    ...
    try:
        ...
        # Get all spam logs
        spam_logs = (
            db.query(RspamdLog.symbols)
            .filter(and_(RspamdLog.time >= cutoff, RspamdLog.is_spam == True))
            .all()
        )
        ...
        return {
            "triggers": [
                {"symbol": symbol, "count": count} for symbol, count in top_symbols
            ]
        }
    except Exception as e:
...
async def get_top_blocked_ips(limit: int = 10, db: Session = Depends(get_db)):
    """
    ...
    try:
        ...
        top_ips = (
            ...
            .filter(NetfilterLog.time >= cutoff)
            .group_by(NetfilterLog.ip)
            .order_by(func.count(NetfilterLog.id).desc())
            .limit(limit)
            .all()
        )
...
async def get_recent_activity(limit: int = 20, db: Session = Depends(get_db)):
    ...
    try:
        recent = (
            db.query(MessageCorrelation)
            .order_by(MessageCorrelation.first_seen.desc())
            .limit(limit)
            .all()
        )
        ...
        return {
            "activity": [
                {
                    "time": format_datetime_utc(msg.first_seen),
                    "sender": msg.sender,
