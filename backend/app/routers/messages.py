    ...
    try:
        ...
        if search:
            ...
            rspamd_subquery = select(RspamdLog.id).where(
                or_(RspamdLog.ip.ilike(search_term), RspamdLog.user.ilike(search_term))
            )
        ...
        offset = (page - 1) * limit
        messages = (
            query.order_by(desc(MessageCorrelation.first_seen))
            .offset(offset)
            .limit(limit * 2)
            .all()
        )
        ...
        for msg in messages:
            ...
            if msg.rspamd_log_id:
                rspamd_log = (
                    db.query(RspamdLog)
                    .filter(RspamdLog.id == msg.rspamd_log_id)
                    .first()
                )
        ...
        return {
            ...
            "page": page,
            "limit": limit,
            "pages": (total_before_filter + limit - 1) // limit
...
async def get_message_full_details(correlation_key: str, db: Session = Depends(get_db)):
    """
    ...
    try:
        ...
        correlation = (
            ...
            .filter(MessageCorrelation.correlation_key == correlation_key)
            .first()
        )
        ...
        if correlation.queue_id:
            ...
            postfix_logs = (
                db.query(PostfixLog)
                .filter(PostfixLog.id.in_(unique_ids))
                .order_by(PostfixLog.time)
                .all()
            )
        ...
        if rspamd_log and rspamd_log.ip:
            ...
            netfilter_logs = (
                ...
                .filter(NetfilterLog.ip == rspamd_log.ip)
                .order_by(NetfilterLog.time.desc())
                .limit(100)
                .all()
        ...
        if rspamd_log and rspamd_log.recipients_smtp:
            recipients = (
                rspamd_log.recipients_smtp
                if isinstance(rspamd_log.recipients_smtp, list)
                else []
            )
