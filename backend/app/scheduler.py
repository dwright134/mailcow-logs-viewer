...
from .correlation import (
    build_postscreen_summary,
    detect_direction,
    generate_postscreen_synthetic_ids,
    get_postscreen_data_from_log,
    is_postscreen_synthetic_queue_id,
...
POSTSCREEN_GROUP_WINDOW_SECONDS = 60
POSTSCREEN_ORPHAN_CLEANUP_MINUTES = 10
POSTSCREEN_GROUP_PROGRAMS = [
    "postfix/postscreen",
    "postfix/dnsblog",
...
_thread_pool_executor = ThreadPoolExecutor(
    max_workers=settings.scheduler_workers, thread_name_prefix="imap_sync"
)
...
def reschedule_interval_jobs():
    ...
    try:
        ...
        if _thread_pool_executor._max_workers != new_workers:
            ...
            _thread_pool_executor = ThreadPoolExecutor(
                max_workers=new_workers, thread_name_prefix="imap_sync"
            )
            try:
        ...
        logger.info(
            f"Rescheduled interval jobs: fetch={settings.fetch_interval}s, correlation={settings.correlation_check_interval}s"
        )
...
def is_version_newer(latest_version: str, current_version: str) -> bool:
    ...
    try:
        ...
        logger.warning(
            "packaging module not available, using fallback version comparison"
        )
        try:
            ...
            def parse_version(version_str: str) -> tuple:
                ...
                return tuple(
                    int(part) for part in parts[:3] + ["0"] * (3 - len(parts[:3]))
                )
            ...
            logger.error(
                f"Failed to compare versions '{latest_version}' and '{current_version}': {e}"
            )
    ...
    try:
        async with httpx.AsyncClient(timeout=10) as client:
            ...
            if response.status_code == 200:
                ...
                app_version_cache["update_available"] = is_version_newer(
                    latest_version, current_version
                )

                logger.info(
                    f"App version check: Current={current_version}, Latest={latest_version}, Update Available={app_version_cache['update_available']}"
                )
...
def _postfix_log_exists(
    ...
    return (
        db.query(PostfixLog.id)
        .filter(
            PostfixLog.time == timestamp,
            PostfixLog.program == program,
            PostfixLog.message == message,
        )
        .first()
        is not None
    )
...
def _delete_postscreen_session(
    db: Session,
    matched_rows: List[PostfixLog],
    queue_id: Optional[str] = None,
    message_id: Optional[str] = None,
):
    row_ids = {row.id for row in matched_rows if row.id}

    if queue_id:
        queued_rows = db.query(PostfixLog).filter(PostfixLog.queue_id == queue_id).all()
        row_ids.update(row.id for row in queued_rows if row.id)

    if row_ids:
        db.query(PostfixLog).filter(PostfixLog.id.in_(row_ids)).delete(
    ...
    if queue_id:
        correlation = (
            ...
            .filter(MessageCorrelation.queue_id == queue_id)
            .first()
        )
...
def _upsert_postscreen_correlation(
    db: Session,
    queue_id: str,
    message_id: str,
    grouped_rows: List[PostfixLog],
) -> Optional[MessageCorrelation]:
    summary = build_postscreen_summary(grouped_rows)
    if not summary:
        return None

    sender = summary.get("sender")
    recipients = summary.get("recipients") or []
    if is_blacklisted(sender) or any(
        is_blacklisted(recipient) for recipient in recipients
    ):
        _delete_postscreen_session(
            db, grouped_rows, queue_id=queue_id, message_id=message_id
        )
        logger.debug("Blacklist: deleted grouped postscreen session %s", queue_id)
        return None

    correlation_key = hashlib.sha256(f"msgid:{message_id}".encode()).hexdigest()
    ordered_rows = sorted(grouped_rows, key=lambda row: row.time)
    first_seen = ordered_rows[0].time
    last_seen = ordered_rows[-1].time
    postfix_log_ids = [row.id for row in ordered_rows if row.id]
    ...
    if not correlation:
        ...
        correlation.correlation_key = correlation_key
        correlation.message_id = message_id
        correlation.queue_id = queue_id
        correlation.sender = sender
        correlation.recipient = recipients[0] if recipients else correlation.recipient
        correlation.subject = "Postscreen reject"
        correlation.direction = "inbound"
        correlation.final_status = "rejected"
        correlation.postfix_log_ids = postfix_log_ids
        correlation.first_seen = first_seen
        correlation.last_seen = last_seen
...
def correlate_postscreen_reject(db: Session, reject_log: PostfixLog):
    postscreen = get_postscreen_data_from_log(reject_log)
    if not postscreen or postscreen.get("event_type") != "reject":
        return

    client_ip = postscreen.get("client_ip")
    client_port = postscreen.get("client_port")
    if not client_ip:
        return

    window_start = reject_log.time - timedelta(seconds=POSTSCREEN_GROUP_WINDOW_SECONDS)
    candidate_rows = (
        db.query(PostfixLog)
        .filter(
            PostfixLog.time >= window_start,
            PostfixLog.time <= reject_log.time,
            PostfixLog.program.in_(POSTSCREEN_GROUP_PROGRAMS),
        )
    ...
    for row in candidate_rows:
        row_postscreen = get_postscreen_data_from_log(row)
        if not row_postscreen or row_postscreen.get("client_ip") != client_ip:
            continue

        event_type = row_postscreen.get("event_type")
        if (
            event_type in {"connect", "dnsbl_rank", "reject"}
            and row_postscreen.get("client_port") != client_port
        ):
            continue
    ...
    if not matched_rows:
        matched_rows = [reject_log]

    connect_rows = [
        row
        for row in matched_rows
        if (get_postscreen_data_from_log(row) or {}).get("event_type") == "connect"
    ]
    session_start_row = connect_rows[0] if connect_rows else matched_rows[0]
    session_start = session_start_row.time
    grouped_rows = [
        row for row in matched_rows if session_start <= row.time <= reject_log.time
    ]

    synthetic_queue_ids = [
        row.queue_id
        for row in grouped_rows
        if is_postscreen_synthetic_queue_id(row.queue_id)
    ]
    synthetic_queue_id = synthetic_queue_ids[0] if synthetic_queue_ids else None

    if synthetic_queue_id:
        existing_grouped_rows = (
            db.query(PostfixLog)
            .filter(PostfixLog.queue_id == synthetic_queue_id)
            .order_by(PostfixLog.time)
            .all()
        )
        existing_message_id = next(
            (row.message_id for row in existing_grouped_rows if row.message_id),
            None,
        )
        queue_id = synthetic_queue_id
        message_id = existing_message_id
        grouped_by_id = {row.id: row for row in existing_grouped_rows if row.id}
        for row in grouped_rows:
            if row.id:
                grouped_by_id[row.id] = row
        grouped_rows = sorted(grouped_by_id.values(), key=lambda row: row.time)
    else:
        queue_id, message_id = generate_postscreen_synthetic_ids(
            client_ip, client_port, session_start
        )

    if not message_id:
        _, message_id = generate_postscreen_synthetic_ids(
            client_ip, client_port, session_start
        )

    _upsert_postscreen_correlation(db, queue_id, message_id, grouped_rows)


def _find_matching_postscreen_rejects(
    db: Session,
    postscreen_log: PostfixLog,
) -> List[PostfixLog]:
    postscreen = get_postscreen_data_from_log(postscreen_log)
    if not postscreen:
        return []
    ...
    reject_window_end = postscreen_log.time + timedelta(
        seconds=POSTSCREEN_GROUP_WINDOW_SECONDS
    )
    candidate_rows = (
        db.query(PostfixLog)
        .filter(
            PostfixLog.time >= postscreen_log.time,
            PostfixLog.time <= reject_window_end,
            PostfixLog.program == "postfix/postscreen",
        )
        .order_by(PostfixLog.time)
        .all()
    )
    ...
    for row in candidate_rows:
        row_postscreen = get_postscreen_data_from_log(row)
        if not row_postscreen:
            continue
        if row_postscreen.get("event_type") != "reject":
            continue
        if row_postscreen.get("client_ip") != client_ip:
            continue
        if (
            event_type in {"connect", "dnsbl_rank", "reject"}
            and row_postscreen.get("client_port") != client_port
            ...
            continue
        matching_rejects.append(row)

    return matching_rejects
...
async def fetch_and_store_postfix():
    ...
    try:
        ...
        sorted_logs = sorted(logs, key=lambda entry: int(entry.get("time", 0)))
        ...
        with get_db_context() as db:
            ...
            postscreen_candidate_logs: List[PostfixLog] = []
            postscreen_reject_logs: List[PostfixLog] = []

            for log_entry in sorted_logs:
                message = log_entry.get("message", "")
                parsed = parse_postfix_message(
                    message,
                    program=log_entry.get("program"),
                )
                queue_id = parsed.get("queue_id")
                postscreen = parsed.get("postscreen")

                if not queue_id or postscreen:
                    continue
                ...
                if is_blacklisted(sender) or is_blacklisted(recipient):
                    ...
                    logger.debug(
                        f"Blacklist: Queue ID {queue_id} marked for deletion (sender={sender}, recipient={recipient})"
                    )
            ...
            if blacklisted_queue_ids:
                deleted_count = (
                    db.query(PostfixLog)
                    .filter(PostfixLog.queue_id.in_(blacklisted_queue_ids))
                ...
                if deleted_count > 0:
                    logger.info(
                        f"[BLACKLIST] Deleted {deleted_count} Postfix logs for {len(blacklisted_queue_ids)} blacklisted queue IDs"
                    )
            ...
            for log_entry in sorted_logs:
                try:
                    ...
                    program = log_entry.get("program")
                    parsed = parse_postfix_message(message, program=program)
                    queue_id = parsed.get("queue_id")
                    postscreen = parsed.get("postscreen")
                    is_postscreen_candidate = bool(postscreen)

                    if (
                        postscreen
                        and postscreen.get("event_type") == "whitelist_lookup"
                    ):
                        seen_postfix.add(unique_id)
                        continue

                    if not queue_id and not is_postscreen_candidate:
                        continue
                    ...
                    if not is_postscreen_candidate:
                        sender = parsed.get("sender")
                        recipient = parsed.get("recipient")
                        if is_blacklisted(sender) or is_blacklisted(recipient):
                            skipped_blacklist += 1
                    ...
                    timestamp = datetime.fromtimestamp(
                        int(log_entry.get("time", 0)), tz=timezone.utc
                    )

                    if _postfix_log_exists(db, timestamp, program, message):
                        seen_postfix.add(unique_id)
                        skipped_count += 1
                        continue
                    ...
                    sender = parsed.get("sender")
                    recipient = parsed.get("recipient")
                    raw_data = dict(log_entry)
                    if postscreen:
                        raw_data["_postscreen"] = postscreen
                    ...
                    if postscreen:
                        postscreen_candidate_logs.append(postfix_log)
                        if postscreen.get("event_type") == "reject":
                            postscreen_reject_logs.append(postfix_log)
            ...
            seen_reject_ids = {log.id for log in postscreen_reject_logs if log.id}
            for postscreen_log in postscreen_candidate_logs:
                for reject_log in _find_matching_postscreen_rejects(db, postscreen_log):
                    if reject_log.id and reject_log.id not in seen_reject_ids:
                        postscreen_reject_logs.append(reject_log)
                        seen_reject_ids.add(reject_log.id)

            for reject_log in postscreen_reject_logs:
                try:
                    correlate_postscreen_reject(db, reject_log)
                except Exception as e:
                    logger.error(f"Error correlating postscreen reject: {e}")
                    db.rollback()
        ...
        with get_db_context() as db:
                ...
                try:
                    ...
                    rspamd_log = RspamdLog(
                        ...
                        is_spam=(
                            ...
                            in ["reject", "add header", "rewrite subject"]
                            or "SPAM_TRAP" in log_entry.get("symbols", {})
                        ),
            ...
            if blacklisted_message_ids:
                correlations_to_delete = (
                    db.query(MessageCorrelation)
                    .filter(MessageCorrelation.message_id.in_(blacklisted_message_ids))
                    .all()
                )
                ...
                if queue_ids_to_delete:
                    deleted_postfix = (
                        db.query(PostfixLog)
                        .filter(PostfixLog.queue_id.in_(queue_ids_to_delete))
                        .delete(synchronize_session=False)
                    )

                    if deleted_postfix > 0:
                        logger.info(
                            f"[BLACKLIST] Deleted {deleted_postfix} Postfix logs linked to blacklisted messages"
                        )

                if deleted_corr > 0:
                    logger.info(
                        f"[BLACKLIST] Deleted {deleted_corr} correlations for blacklisted message IDs"
                    )
...
def parse_netfilter_message(
    message: str, priority: Optional[str] = None
) -> Dict[str, Any]:
    result = {}
    ...
    if not result.get("ip"):
        banned_match = re.search(
            r"Ban(?:ned|ning)\s+(\d+\.\d+\.\d+\.\d+)", message, re.IGNORECASE
        )
        if banned_match:
    ...
    if not result.get("ip"):
        cidr_match = re.search(
            r"Ban(?:ned|ning)\s+(\d+\.\d+\.\d+\.\d+/\d+)", message, re.IGNORECASE
        )
        if cidr_match:
    ...
    # This catches denylist messages like "Added host/network 175.157.10.170/32 to denylist".
    if not result.get("ip"):
...
async def fetch_and_store_netfilter():
    ...
    try:
        logger.debug(
            f"[NETFILTER] Starting fetch (count: {settings.fetch_count_netfilter})"
        )
        logs = await mailcow_api.get_netfilter_logs(
            count=settings.fetch_count_netfilter
        )
        ...
        with get_db_context() as db:
            ...
            if new_count > 0:
                logger.info(
                    f"[OK] Imported {new_count} Netfilter logs (skipped {skipped_count} duplicates)"
                )
            elif skipped_count > 0:
                logger.debug(
                    f"[NETFILTER] All {skipped_count} logs were duplicates, nothing new to import"
                )
    ...
    try:
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                ...
                logger.error(
                    f"[ERROR] {log_type} fetch failed: {result}", exc_info=result
                )
    ...
    try:
        with get_db_context() as db:
            ...
            for email in blacklist:
                logs_with_blacklisted_recipient = (
                    db.query(PostfixLog)
                    .filter(
                        PostfixLog.recipient == email, PostfixLog.queue_id.isnot(None)
                    )
                    .all()
                )
            ...
            if deleted_count > 0:
                logger.info(
                    f"[CLEANUP] Cleaned up {deleted_count} Postfix logs from {len(blacklisted_queue_ids)} blacklisted BCC queues"
                )
        ...
        with get_db_context() as db:
            ...
            for rspamd_log in uncorrelated_rspamd:
                try:
                    ...
                    logger.warning(
                        f"Correlation failed for rspamd {rspamd_log.id}: {e}"
                    )
                    db.rollback()
                    continue
...
def correlate_single_message(
    ...
    try:
        ...
        correlation = MessageCorrelation(
            ...
            postfix_log_ids=[plog.id for plog in all_postfix_logs]
            if all_postfix_logs
            else [],
            first_seen=first_seen,
            last_seen=now,
        ...
        logger.debug(
            f"Created correlation for {message_id[:40]}... (queue: {queue_id}, {len(all_postfix_logs)} postfix logs)"
        )
        return correlation
        ...
        with get_db_context() as db:
            ...
            for correlation in incomplete:
                try:
                    ...
                    postfix_with_msgid = (
                        db.query(PostfixLog)
                        .filter(PostfixLog.message_id == correlation.message_id)
                        .all()
                    )
                    ...
                    for plog in all_postfix:
                        if plog.status:
                            if plog.status in ["bounced", "rejected"]:
                                ...
                                plog.status == "deferred"
                                and correlation.final_status
                                not in ["bounced", "rejected"]
                            ):
                                correlation.final_status = plog.status
                    ...
                    logger.warning(
                        f"Failed to complete correlation {correlation.id}: {e}"
                    )
                    continue
...
async def expire_old_correlations():
    ...
    try:
        with get_db_context() as db:
            ...
            if expired_count > 0:
                logger.info(
                    f"[EXPIRED] Marked {expired_count} correlations as expired (older than {settings.max_correlation_age_minutes}min)"
                )
        ...
        with get_db_context() as db:
            ...
            correlations_to_check = (
                ...
                .filter(
                    ...
                    or_(
                        ...
                        MessageCorrelation.final_status.notin_(
                            ["delivered", "bounced", "rejected", "expired"]
                        ),
                    ),
                )
                .limit(500)
            ...
            for correlation in correlations_to_check:
                try:
                    ...
                    all_postfix = (
                        db.query(PostfixLog)
                        .filter(PostfixLog.queue_id == correlation.queue_id)
                        .all()
                    )
                    ...
                    for plog in all_postfix:
                        if plog.status:
                            if plog.status in ["bounced", "rejected"]:
                                ...
                                if new_final_status not in [
                                    "bounced",
                                    "rejected",
                                    "delivered",
                                ]:
                                    new_final_status = "delivered"
                            elif plog.status == "deferred" and new_final_status not in [
                                "bounced",
                                "rejected",
                                "delivered",
                            ]:
                    ...
                    for plog in all_postfix:
                        if (
                            not plog.correlation_key
                            or plog.correlation_key != correlation.correlation_key
                        ):
                            plog.correlation_key = correlation.correlation_key
                    ...
                    if (
                        new_final_status
                        and new_final_status != correlation.final_status
                    ) or ids_added > 0:
                        ...
                        if ids_added > 0:
                            logger.debug(
                                f"Updated correlation {correlation.id}: added {ids_added} logs, status {old_status} -> {new_final_status}"
                            )
                        else:
                            logger.debug(
                                f"Updated final_status for correlation {correlation.id} ({correlation.message_id[:40] if correlation.message_id else 'no-id'}...): {old_status} -> {new_final_status}"
                            )

                except Exception as e:
                    logger.warning(
                        f"Failed to update final_status for correlation {correlation.id}: {e}"
                    )
                    continue
            ...
            if updated_count > 0:
                logger.info(
                    f"[STATUS] Updated final_status for {updated_count} correlations"
                )
...
async def check_monitored_hosts_job(
    force: bool = False, send_notification: bool = True
):
    """
    ...
    try:
        ...
        logger.info(
            f"Starting blacklist check job (send_notification={send_notification})..."
        )
        ...
        with get_db_context() as db:
            ...
            for h in db_hosts:
                monitored_hosts.append({"hostname": h.hostname, "source": h.source})
        ...
        if not monitored_hosts:
            ...
            if server_ip:
                with get_db_context() as db:
                    ...
                    monitored_hosts.append({"hostname": server_ip, "source": "system"})

        if not monitored_hosts:
            logger.warning("Cannot check blacklists: No monitored hosts available")
            update_job_status(
                "blacklist_check", "failed", "No monitored hosts available"
            )
            return
        ...
        try:
            for i, host in enumerate(monitored_hosts):
                ...
                logger.info(
                    f"Processing host {i + 1}/{len(monitored_hosts)}: {hostname} (Source: {source})"
                )
                target_ip = hostname
                ...
                logger.info(
                    f"Finished blacklist check for {target_ip}. Results: listed={results.get('listed_count')}"
                )
                total_checks += 1
                ...
                if i < len(monitored_hosts) - 1:
                    ...
                    update_batch_status(
                        f"Cooldown 10s before checking {monitored_hosts[i + 1]['hostname']}..."
                    )
                    await asyncio.sleep(10)

            logger.info(
                f"Blacklist check complete for {total_checks} hosts. {len(listed_hosts)} listed."
            )
            ...
            for host_data in listed_hosts:
                ...
                for res in results:
                    if (
                        res.get("listed")
                        and res.get("name") not in IGNORED_NOTIFICATION_BLACKLISTS
                    ):
            ...
            if listed_hosts and send_notification and not should_alert:
                logger.info(
                    "Hosts are listed, but only on ignored blacklists (e.g. UCEPROTECT). Suppressing notification."
                )
            ...
            if listed_hosts and send_notification and should_alert:
                ...
                blacklist_alert_email = (
                    settings.blacklist_alert_email
                    if hasattr(settings, "blacklist_alert_email")
                    else None
                )
                notification_email = get_notification_email(blacklist_alert_email)

                logger.info(
                    f"Found listed hosts. Attempting to send alert to: {notification_email} (Source: {blacklist_alert_email})"
                )

                if notification_email:
                    # Build aggregated email content
                    subject = (
                        f"⚠️ ALERT: {len(listed_hosts)} Host(s) Listed on Blacklists"
                    )
                    ...
                    for host_data in listed_hosts:
                        ...
                        listed_bls = [
                            r for r in results.get("results", []) if r.get("listed")
                        ]
                        bl_text_list = "\n".join(
                            [f"  - {bl['name']} ({bl['zone']})" for bl in listed_bls]
                        )

                        text_content += f"Host: {display_name}{extra_info}\n"
                        text_content += (
                            f"Listed on: {listed_count}/{total_bl} blacklists\n"
                        )
                        ...
                        bl_html_list = "".join(
                            [
                                f'<li><strong>{bl["name"]}</strong> - {bl["zone"]} (<a href="{bl.get("info_url", "#")}">lookup</a>)</li>'
    ...
    """
                    ...
                    sent = send_notification_email(
                        notification_email, subject, text_content, html_content
                    )
                    if sent:
                        logger.info(f"Blacklist alert sent to {notification_email}")
                    else:
                        logger.error(
                            f"Failed to send blacklist alert to {notification_email}. Check SMTP settings."
                        )
                        ...
                        logger.warning(
                            f"SMTP Host: {svc.host}:{svc.port}, User: {svc.user}, From: {svc.from_address}"
                        )
                else:
                    logger.error(
                        "Blacklist alert: No notification email configured (both blacklist_alert_email and admin_email are missing)"
                    )
            elif listed_hosts and not send_notification:
                logger.info(
                    f"Blacklist check completed with listed hosts ({len(listed_hosts)}), but notification is suppressed (by job param)."
                )
    ...
    """
                        ...
                        still_listed = ", ".join(
                            h["hostname"] for h in actionable_listed_hosts
                        )
                        text_content = (
    ...
    """
                    ...
                    if sent:
                        logger.info(
                            f"Blacklist count change notification sent to {notification_email} (was {prev_listed}, now {actionable_count})"
                        )
                    else:
                        logger.error(
                            f"Failed to send blacklist count change notification to {notification_email}"
                        )
                else:
                    logger.error(
                        "Blacklist count change: No notification email configured."
                    )
...
async def dmarc_imap_sync_job():
    ...
    try:
        ...
        with SessionLocal() as db:
            db.query(DMARCSync).filter(DMARCSync.status == "running").update(
                {"status": "failed", "error_message": "Stale job cleaned by scheduler"}
            )
            db.commit()
    ...
    try:
        ...
        result = await asyncio.get_event_loop().run_in_executor(
            _thread_pool_executor, sync_dmarc_reports_from_imap, "auto"
        )
        ...
        if result.get("status") == "error":
            ...
            logger.info(
                f"DMARC IMAP sync completed: {result.get('reports_created', 0)} created"
            )
...
async def cleanup_old_logs():
    ...
    try:
        with get_db_context() as db:
            postscreen_cutoff = datetime.now(timezone.utc) - timedelta(
                minutes=POSTSCREEN_ORPHAN_CLEANUP_MINUTES
            )
            orphan_candidates = [
                row
                for row in db.query(PostfixLog)
                .filter(
                    PostfixLog.time < postscreen_cutoff,
                    PostfixLog.queue_id.is_(None),
                )
                .all()
                if get_postscreen_data_from_log(row)
            ]
            orphan_candidate_ids = [row.id for row in orphan_candidates if row.id]
            orphan_deleted = 0
            if orphan_candidate_ids:
                orphan_deleted = (
                    db.query(PostfixLog)
                    .filter(PostfixLog.id.in_(orphan_candidate_ids))
                    .delete(synchronize_session=False)
                )
            ...
            postfix_deleted = (
                db.query(PostfixLog).filter(PostfixLog.time < cutoff_date).delete()
            )

            rspamd_deleted = (
                db.query(RspamdLog).filter(RspamdLog.time < cutoff_date).delete()
            )

            netfilter_deleted = (
                db.query(NetfilterLog).filter(NetfilterLog.time < cutoff_date).delete()
            )

            correlation_deleted = (
                db.query(MessageCorrelation)
                .filter(MessageCorrelation.first_seen < cutoff_date)
                .delete()
            )
...
async def cleanup_old_dmarc_reports():
    ...
    try:
        with get_db_context() as db:
            ...
            old_dmarc_ids = [
                row[0]
                for row in db.query(DMARCReport.id)
                .filter(DMARCReport.created_at < cutoff_date)
                .all()
            ]
            dmarc_records_deleted = 0
            if old_dmarc_ids:
                dmarc_records_deleted = (
                    db.query(DMARCRecord)
                    .filter(DMARCRecord.dmarc_report_id.in_(old_dmarc_ids))
                    .delete(synchronize_session=False)
                )
                dmarc_reports_deleted = (
            ...
            old_tls_ids = [
                row[0]
                for row in db.query(TLSReport.id)
                .filter(TLSReport.created_at < cutoff_date)
                .all()
            ]
            tls_policies_deleted = 0
            if old_tls_ids:
                tls_policies_deleted = (
                    db.query(TLSReportPolicy)
                    .filter(TLSReportPolicy.tls_report_id.in_(old_tls_ids))
                    .delete(synchronize_session=False)
                )
                tls_reports_deleted = (
                    db.query(TLSReport)
...
def cleanup_blacklisted_data():
    ...
    logger.info(
        f"[BLACKLIST] Running startup cleanup for {len(blacklist)} blacklisted emails..."
    )
    ...
    try:
        with get_db_context() as db:
            ...
            for email in blacklist:
                ...
                deleted = (
                    db.query(MessageCorrelation)
                    .filter(MessageCorrelation.sender.ilike(email))
                    .delete(synchronize_session=False)
                )
                ...
                deleted = (
                    db.query(MessageCorrelation)
                    .filter(MessageCorrelation.recipient.ilike(email))
                    .delete(synchronize_session=False)
                )
                total_deleted += deleted
            ...
            if total_deleted > 0:
                logger.info(
                    f"[BLACKLIST] Deleted {total_deleted} correlations with blacklisted emails"
                )
            ...
            for email in blacklist:
                ...
                postfix_with_sender = (
                    db.query(PostfixLog.queue_id)
                    .filter(
                        PostfixLog.sender.ilike(email), PostfixLog.queue_id.isnot(None)
                    )
                    .distinct()
                    .all()
                )
            ...
            if blacklisted_queue_ids:
                ...
                if deleted_postfix > 0:
                    logger.info(
                        f"[BLACKLIST] Deleted {deleted_postfix} Postfix logs from {len(blacklisted_queue_ids)} blacklisted queue IDs"
                    )

                # Also delete any remaining correlations for these queue IDs
                deleted_corr = (
                    db.query(MessageCorrelation)
                    .filter(MessageCorrelation.queue_id.in_(blacklisted_queue_ids))
                    .delete(synchronize_session=False)
                )

                if deleted_corr > 0:
                    logger.info(
                        f"[BLACKLIST] Deleted {deleted_corr} additional correlations"
                    )
            ...
            for email in blacklist:
                ...
                deleted = (
                    db.query(RspamdLog)
                    .filter(RspamdLog.sender_smtp.ilike(email))
                    .delete(synchronize_session=False)
                )
            ...
            if deleted_rspamd > 0:
                logger.info(
                    f"[BLACKLIST] Deleted {deleted_rspamd} Rspamd logs with blacklisted senders"
                )
    ...
    try:
        ...
        for domain_data in domains:
            ...
            try:
                ...
                with get_db_context() as db:
                    await save_dns_check_to_db(
                        db, domain_name, dns_data, is_full_check=True
                    )
...
async def sync_local_domains():
    ...
    try:
        ...
        all_domains = list(
            dict.fromkeys((active_domains or []) + (alias_domains or []))
        )
        if all_domains:
            set_cached_active_domains(all_domains)
            logger.info(
                f"✓ Local domains synced: {len(active_domains or [])} primary, {len(alias_domains or [])} alias → {len(all_domains)} total"
            )
...
async def update_mailbox_statistics():
    ...
    try:
        ...
        api_mailbox_usernames = {
            mb.get("username") for mb in mailboxes if mb.get("username")
        }
        ...
        with get_db_context() as db:
            ...
            for db_mb in db_mailboxes:
                if db_mb.username not in api_mailbox_usernames:
                    if db_mb.active:  # Only log and count if it was previously active
                        logger.info(
                            f"Marking deleted mailbox as inactive: {db_mb.username}"
                        )
                        db_mb.active = False
            ...
            for mb in mailboxes:
                try:
                    ...
                    existing = (
                        db.query(MailboxStatistics)
                        .filter(MailboxStatistics.username == username)
                        .first()
                    )
                    ...
                    rl_value = (
                        safe_int(rl_value_raw)
                        if rl_value_raw not in (None, "", "-", "- ")
                        else None
                    )
                    rl_frame = (
                        rl_frame_raw
                        if rl_frame_raw not in (None, "", "-", "- ")
                        else None
                    )
                    ...
                    if existing:
                        ...
                        existing.percent_in_use = safe_float(
                            mb.get("percent_in_use"), 0.0
                        )
                        existing.messages = safe_int(mb.get("messages"), 0)
                        existing.active = mb.get("active", 1) == 1
                        existing.last_imap_login = (
                            safe_int(mb.get("last_imap_login"), 0) or None
                        )
                        existing.last_pop3_login = (
                            safe_int(mb.get("last_pop3_login"), 0) or None
                        )
                        existing.last_smtp_login = (
                            safe_int(mb.get("last_smtp_login"), 0) or None
                        )
                        ...
                        new_mailbox = MailboxStatistics(
                            ...
                            last_imap_login=safe_int(mb.get("last_imap_login"), 0)
                            or None,
                            last_pop3_login=safe_int(mb.get("last_pop3_login"), 0)
                            or None,
                            last_smtp_login=safe_int(mb.get("last_smtp_login"), 0)
                            or None,
                    ...
                    logger.error(
                        f"Error processing mailbox {mb.get('username', 'unknown')}: {e}"
                    )
                    continue

            db.commit()
            logger.info(
                f"✓ Mailbox statistics updated: {updated} updated, {created} created, {deleted} deactivated"
            )
            update_job_status("mailbox_stats", "success")
...
async def update_alias_statistics():
    ...
    try:
        ...
        api_alias_addresses = {
            alias.get("address") for alias in aliases if alias.get("address")
        }
        ...
        with get_db_context() as db:
            ...
            for db_alias in db_aliases:
                if db_alias.alias_address not in api_alias_addresses:
                    if (
                        db_alias.active
                    ):  # Only log and count if it was previously active
                        logger.info(
                            f"Marking deleted alias as inactive: {db_alias.alias_address}"
                        )
            ...
            for alias in aliases:
                try:
                    ...
                    domain = (
                        alias_address.split("@")[-1] if "@" in alias_address else ""
                    )
                    ...
                    existing = (
                        db.query(AliasStatistics)
                        .filter(AliasStatistics.alias_address == alias_address)
                        .first()
                    )
                    ...
                    logger.error(
                        f"Error processing alias {alias.get('address', 'unknown')}: {e}"
                    )
                    continue

            db.commit()
            logger.info(
                f"✓ Alias statistics updated: {updated} updated, {created} created, {deleted} deactivated"
            )
            update_job_status("alias_stats", "success")
        ...
        def resolve_and_validate(host_input: str, source_type: str) -> Optional[tuple]:
            ...
            try:
                ...
                if ip_obj.is_private or ip_obj.is_loopback:
                    logger.info(
                        f"Skipping private/loopback IP: {ip_str} ({host_clean})"
                    )
                    return None
...
def start_scheduler():
    ...
    try:
        ...
        if is_license_configured():
            ...
            logger.info(
                "   [GEOIP] Initial GeoIP check scheduled (60 seconds after startup)"
            )
        else:
            logger.info(
                "   [GEOIP] MaxMind license key not configured, GeoIP features disabled"
            )
        ...
        if settings.dmarc_imap_enabled:
            ...
            logger.info(
                f"Scheduled DMARC IMAP sync job (interval: {settings.dmarc_imap_interval}s)"
            )
        ...
        logger.info(
            f"   [COMPLETE] Incomplete correlations: every {settings.correlation_check_interval}s"
        )
        logger.info(
            f"   [STATUS] Update final status: every {settings.correlation_check_interval}s (max age: {settings.max_correlation_age_minutes}min)"
        )
        logger.info(
            f"   [EXPIRE] Old correlations: every 60s (expire after {settings.max_correlation_age_minutes}min)"
        )
        ...
        if settings.dmarc_imap_enabled:
            logger.info(
                f"   [DMARC] IMAP sync: every {settings.dmarc_imap_interval // 60} minutes"
            )
        else:
